...
class Scanner(threading.Thread):
    ...
    def run(self):
        ...
        # if two or more files have same hash and size they are added to the
        # queue
        for files in [flist for flist in filesizes.values() if len(flist) > 1]:
            # run over all files in dir with the same size if there is more
            # then one
            duplicates = {}
        ...
        while True:
            try:
                item = self._queue.get(
                    True,
                    0.03)  # seems to be a good time value
            except Queue.Empty:
